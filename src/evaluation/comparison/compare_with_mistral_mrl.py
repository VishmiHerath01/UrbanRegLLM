...
from rag_pipelines.pretrained.mistralWmrl_em import rag_pipeline as mrl_rag_pipeline
...
def compare_with_mistral_mrl():
    ...
    for query in tqdm(test_queries, desc="Processing queries"):
        ...
        # Generate answer using MRL embeddings
        try:
    ...
    answers_file = os.path.join(RESULTS_DIR, "mistral_mrl_answers.json")
    with open(answers_file, 'w') as f:
        json.dump(answers, f, indent=2)
    
    # Generate CSV for easier analysis
    csv_file = os.path.join(RESULTS_DIR, "mistral_mrl_comparison.csv")
    with open(csv_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Query", "Regular Answer", "MRL Answer", "Regular Search Time", "MRL Search Time", "Regular Tokens", "MRL Tokens"])
...
def generate_markdown_report(answers):
    ...
    report_path = os.path.join(RESULTS_DIR, "mistral_mrl_comparison_report.md")
    
    with open(report_path, 'w') as f:
        f.write("# Mistral with MRL Embeddings Comparison Report\n\n")
        
        f.write("## Overview\n\n")
        f.write("This report compares two approaches for retrieving and generating answers to questions about Sri Lankan Urban Development Regulations:\n\n")
        f.write("1. **Regular Embeddings**: Using the full-dimensional legal embeddings model\n")
        f.write(f"2. **MRL Embeddings**: Using Matryoshka Representation Learning embeddings ({MRL_DIMENSION}d)\n\n")
        ...
        if avg_regular_search_time != "N/A" and avg_mrl_search_time != "N/A":
            f.write(f"| Search Time | {avg_regular_search_time:.4f}s | {avg_mrl_search_time:.4f}s |\n")
        else:
            f.write(f"| Search Time | {avg_regular_search_time} | {avg_mrl_search_time} |\n")
            
        if avg_regular_gen_time != "N/A" and avg_mrl_gen_time != "N/A":
            f.write(f"| Generation Time | {avg_regular_gen_time:.2f}s | {avg_mrl_gen_time:.2f}s |\n")
        else:
            f.write(f"| Generation Time | {avg_regular_gen_time} | {avg_mrl_gen_time} |\n")
            
        if avg_regular_tokens != "N/A" and avg_mrl_tokens != "N/A":
            f.write(f"| Tokens Used | {avg_regular_tokens:.1f} | {avg_mrl_tokens:.1f} |\n\n")
        else:
            f.write(f"| Tokens Used | {avg_regular_tokens} | {avg_mrl_tokens} |\n\n")
        ...
        regular_dim = 768  # Typical dimension for full embeddings
        mrl_dim = MRL_DIMENSION
        memory_reduction = (1 - (mrl_dim / regular_dim)) * 100
        f.write(f"**Memory Usage Reduction with MRL:** {memory_reduction:.2f}%\n\n")
        ...
        for query, data in answers.items():
            ...
            if data["regular"]["metrics"]:
                ...
                if search_time != "N/A":
                    ...
                    f.write(f"**Metrics:** Search Time: {search_time}, Tokens: {tokens}\n\n")
