...
def load_llm(model_path):
    ...
    try:
        ...
        logging.info(f"TinyLlama model loaded successfully on device: {model.device}")
        return {"model": model, "tokenizer": tokenizer}
    except Exception as e:
        logging.error(f"Error loading TinyLlama model: {str(e)}")
        return None

def generate_answer(llm, query, retrieved_docs, max_tokens=1024, temperature=0.1):
    """
    Generate an answer using the TinyLlama model based on retrieved documents.
    """
    # Create context from retrieved documents
    context = "\n\n".join([f"Document {i+1}:\n{doc['content']}" for i, doc in enumerate(retrieved_docs)])
    
    # Create prompt - use the correct format for TinyLlama chat models
    prompt = f"""<s>[INST] You are an assistant specializing in Sri Lankan urban development regulations. Answer the following question based on the context provided.
...
Answer the question thoroughly and list any applicable rules, clauses, or sections. If the exact answer isn't in the context but you can reasonably infer it from the provided regulations, provide that inference. If you mention regulation numbers or refer to specific rules, cite them directly from the context. Only say "I don't have information about that in the regulations" if the context has absolutely no relevant information. [/INST]"""


    # Extract model and tokenizer from the dictionary
    model = llm["model"]
    ...
    output = model.generate(
        **inputs,
        ...
        num_beams=1  # Use greedy decoding for more focused responses
    )
    
    # Decode the output and extract only the answer part
    generated_text = tokenizer.decode(output[0], skip_special_tokens=True)
    answer = generated_text[len(tokenizer.decode(inputs["input_ids"][0], skip_special_tokens=True)):].strip()
    
    end_time = time.time()
    
    # Create response object similar to the original Mistral implementation
    generation_info = {
        "tokens": len(output[0]) - len(inputs["input_ids"][0]),
...
def rag_pipeline(query, embedding_model_path, llm_model_path, index_path="legal_faiss_index", k=5):
    ...
    2. Generate an answer using the TinyLlama model
    """
...
if __name__ == "__main__":
    ...
    LLM_MODEL_PATH = "/Users/vishmiherath/Documents/FYP/tinyllama_finetuned/full_model"  # Path to fine-tuned TinyLlama model
